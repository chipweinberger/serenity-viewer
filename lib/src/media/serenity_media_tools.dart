...
extension _SerenityShellMediaTools on _SerenityShellState {
  _SharedVideoControllerEntry? _sharedVideoControllerForWindow(AssetWindowState window, {required bool isLoaded}) {
    ...
    final existing = _mediaControllerCache.sharedVideoControllers[window.asset.id];
    if (existing != null) {
      if (existing.path == path) {
        return existing;
      }
      _mediaControllerCache.sharedVideoControllers.remove(window.asset.id);
    ...
    _mediaControllerCache.sharedVideoControllers[window.asset.id] = entry;
    return entry;
  }
  ...
  void _syncSharedVideoControllers(SerenityLoadPlan loadPlan) {
    ...
    final staleIds = _mediaControllerCache.sharedVideoControllers.keys
        .where((id) => !retainedVideoIds.contains(id))
        .toList();
    for (final id in staleIds) {
      final removed = _mediaControllerCache.sharedVideoControllers.remove(id);
      if (removed != null) {
